...
【时间线提取（第二梯队）】
5. timeline_events: 本章发生的关键事件及时间戳，每项包含：
   - event: 事件描述（简短）
...
【故事线提取（第三梯队）】
6. advanced_storylines: 本章推进的已有故事线，每项包含：
   - storyline_id: 故事线ID（如果能识别）或 storyline_name（故事线名称）
   - progress_summary: 本章在该线上的进展（1-2句话）

7. new_storylines: 本章引入的新故事线，每项包含：
   - name: 故事线名称（如"复仇主线"、"师徒暗线"）
   - type: 类型（"main"=主线、"sub"=支线、"hidden"=暗线）
   - description: 故事线描述（1-2句话）
   注意：只在真正开启新线索时提取，不要过度解读

请返回以下七个键，不要增加其他顶层字段：new_characters, character_actions, relationship_changes, events, timeline_events, advanced_storylines, new_storylines"""
