...
3. **pacing_guide 标签单调**：只有"起源/深渊/决战"三种标签，6部结构也用同一套标签

结果：1000 章超长篇的结构规划，LLM 收到的 pacing_guide 描述完全不符合该题材的正确节奏。

## What Changes

- 修改 `_calculate_chapter_distribution` 支持 `genre` 参数，按题材返回不同比例曲线（当前支持：玄幻、历史）
